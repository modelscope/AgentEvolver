...
from context_manager_templates.cmt_linear_think import ExtendedMessage, CMTLinear, LinearThinkCMT
...
def _dashscope_key_lists() -> Tuple[List[str], List[str]]:
    """DashScope keys: prefer env; otherwise documented placeholders (not real secrets)."""
    regular = _parse_csv_env("DASHSCOPE_API_KEYS_REGULAR") or _parse_csv_env("DASHSCOPE_API_KEYS")
    if not regular:
        single = os.environ.get("DASHSCOPE_API_KEY", "").strip()
        if single:
            regular = [single]
    if not regular:
        regular = ["<DASHSCOPE_API_KEY_1>", "<DASHSCOPE_API_KEY_2>"]
    backup = _parse_csv_env("DASHSCOPE_API_KEYS_BACKUP")
    if not backup:
        backup = ["<DASHSCOPE_API_KEY_BACKUP>"]
...
def construct_alien_llm_chat_fn(config, rollout_config):
    def alien_llm_chat_fn(messages, request_id=""):
        ...
        regular_key_list, backup_key_list = _dashscope_key_lists()
