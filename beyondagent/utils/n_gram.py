...
def preserve_chinese(text):
    ...
    # Concatenate matched characters
...
def get_repetition_penalty_reward(ngram_size: int, max_penalty: float):
    ...
    def repetition_penalty_reward(completions, **kwargs) -> float:
        ...
        for completion in contents:
            ...
            # total is fixed (segment count after tokenization)
            # all distinct n-grams: len(ngrams)==total -> scaling~=0, reward=0
            # all identical n-grams: len(ngrams)==1 -> scaling~=1, reward=-max_penalty
