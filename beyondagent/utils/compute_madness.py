...
# Regex fragments per whitelist category
...
def build_pattern(white_list):
    """Build a regex from whitelist category names."""
    allowed_parts = ['\x00-\x7F']  # all ASCII
    for name in white_list:
        if name in WHITE_LIST_REGEX_PARTS:
            allowed_parts.append(WHITE_LIST_REGEX_PARTS[name])
    # Merge allowed ranges into one negated character class
    allowed_class = ''.join(allowed_parts)
    pattern = f'[^{allowed_class}]'  # match any character not in the whitelist
...
if __name__ == "__main__":
    # smoke examples (uncomment to run manually)
    # print(repetition_penalty_reward_scalar("Hello world!"))  # 0
    # print(repetition_penalty_reward_scalar("Hello world! 😄"))  # 0
    # print(repetition_penalty_reward_scalar("Hello world! Hello world!"))  # -1.0
    # print(repetition_penalty_reward_scalar("non-ASCII-only text"))  # -1.0 if policy rejects
