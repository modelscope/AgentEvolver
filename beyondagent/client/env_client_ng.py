...
# Default: cwd-relative; override with CLIENT_LOG_PATH. Legacy internal example (placeholder): `<INTERNAL_BASE_PATH>/rl_log/error.out`
_DEFAULT_CLIENT_LOG_PATH = os.path.join(os.getcwd(), "logs", "rl_log", "error.out")
LOG_PATH = os.environ.get("CLIENT_LOG_PATH", _DEFAULT_CLIENT_LOG_PATH)
...
def safe_log(msg: str):
    try:
        ...
        pass  # ignore log write failures so RL training is not blocked
...
class EnvClient:
    ...
    def step(
        ...
        res['state'] = res['state'][0] if "state" in res else res['data']["state"]
        return res
