...
def send_train_message(message: str):
    import requests, os     # optional SMS notification for training status
    ...
    if os.getenv("ALIYUN_SMS_SERVICE") and os.getenv("SMS"):
        ...
        try: requests.post(json={"phone_numbers": "your-phone-number", "server_code": "DLC", "error": message, "error_level": "无"}, url=os.getenv("ALIYUN_SMS_SERVICE", "http://localhost:8000/send-sms"), headers={"Content-Type": "application/json"})
...
from beyondagent.utils.process_dataset import create_rl_dataset, create_rl_sampler
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        ray.init(
            ...
            include_dashboard=False,  # avoid opentelemetry<1.30 vs Ray explicit_bucket_boundaries_advisory mismatch
        )
    import atexit
    atexit.register(lambda: send_train_message("Note: training finished"))  # SMS if ALIYUN_SMS_SERVICE / SMS env vars are set
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        from beyondagent.module.trainer.ba_ray_trainer import ResourcePoolManager, Role
