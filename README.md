...
| `requirements_NewVerl.txt` | Ultra-short install reminder; the English walkthrough lives under **Environment setup** below. |
...
The snippet below tracks the current **`launcher/qwen3_bfcl/qwen3-seeupo-bfcl.yaml`**. Other benchmarks mirror the same **`algorithm`** block; adjust **`env_service`**, **`trainer.nnodes`**, paths, and model checkpoints for your run.

#### 1) `algorithm` — advantage, loss, sequential SeeUPO core
...
#### 2) `env_service` — where rollouts talk to the benchmark
...
#### 3) `data` — batching and file paths
...
#### 4) `trainer` — hardware & logging (abridged)
...
#### 5) `actor_rollout_ref` — optimization, rollout, model
...
**Prerequisites:** complete **Environment setup** (benchmark sandboxes + training Conda env, optionally **`sync_env_with_yaml.py`**) in the section above. Activate your **training** environment (`conda activate seeupo` or your name), `cd` to the repo root, and ensure CUDA/driver match your vLLM build.
